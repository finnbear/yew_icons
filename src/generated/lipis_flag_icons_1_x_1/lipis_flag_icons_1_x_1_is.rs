use yew :: { Callback , Html , MouseEvent } ; # [inline (never)] pub fn lipis_flag_icons_1_x_1_is (width : String , height : String , onclick : Option < Callback < MouseEvent >>) -> Html { yew :: html ! { < svg xmlns = "http://www.w3.org/2000/svg" data - license = "From https://github.com/lipis/flag-icons - Licensed under MIT" { width } { height } { onclick } fill = "currentColor" id = "flag-icons-is" viewBox = "0 0 512 512" > < defs > { "
    " } < clipPath id = "is-a" > { "
      " } < path fill - opacity = ".7" d = "M85.4 0h486v486h-486z" /> { "
    " } </ clipPath > { "
  " } </ defs > { "
  " } < g fill - rule = "evenodd" stroke - width = "0" transform = "translate(-90) scale(1.0535)" > { "
