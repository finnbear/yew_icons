use yew :: { Callback , Html , MouseEvent } ; # [inline (never)] pub fn lipis_flag_icons_1_x_1_sg (width : String , height : String , onclick : Option < Callback < MouseEvent >>) -> Html { yew :: html ! { < svg xmlns = "http://www.w3.org/2000/svg" data - license = "From https://github.com/lipis/flag-icons - Licensed under MIT" { width } { height } { onclick } fill = "currentColor" id = "flag-icons-sg" viewBox = "0 0 512 512" > < defs > { "
    " } < clipPath id = "sg-a" > { "
      " } < path fill - opacity = ".7" d = "M27.7 0h708.6v708.7H27.7z" /> { "
    " } </ clipPath > { "
  " } </ defs > { "
  " } < g fill - rule = "evenodd" transform = "translate(-20) scale(.72249)" > { "
