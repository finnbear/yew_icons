use yew :: { Callback , Html , MouseEvent } ; # [inline (never)] pub fn lipis_flag_icons_4_x_3_nr (width : String , height : String , onclick : Option < Callback < MouseEvent >>) -> Html { yew :: html ! { < svg xmlns = "http://www.w3.org/2000/svg" data - license = "From https://github.com/lipis/flag-icons - Licensed under MIT" { width } { height } { onclick } fill = "currentColor" id = "flag-icons-nr" viewBox = "0 0 640 480" > < defs > { "
    " } < clipPath id = "nr-a" > { "
      " } < path fill - opacity = ".7" d = "M-54.7 0H628v512H-54.7z" /> { "
    " } </ clipPath > { "
  " } </ defs > { "
  " } < g fill - rule = "evenodd" stroke - width = "1pt" transform = "translate(51.3) scale(.9375)" > { "
