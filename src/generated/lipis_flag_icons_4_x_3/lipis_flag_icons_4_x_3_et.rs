use yew :: { Callback , Html , MouseEvent } ; # [inline (never)] pub fn lipis_flag_icons_4_x_3_et (width : String , height : String , onclick : Option < Callback < MouseEvent >>) -> Html { yew :: html ! { < svg xmlns = "http://www.w3.org/2000/svg" data - license = "From https://github.com/lipis/flag-icons - Licensed under MIT" { width } { height } { onclick } fill = "currentColor" id = "flag-icons-et" viewBox = "0 0 640 480" > < defs > { "
    " } < clipPath id = "et-a" > { "
      " } < path fill - opacity = ".7" d = "M-61.3 0h682.7v512H-61.3z" /> { "
    " } </ clipPath > { "
  " } </ defs > { "
  " } < g fill - rule = "evenodd" stroke - width = "1pt" transform = "translate(57.5) scale(.94)" > { "
