use yew :: { Callback , Html , MouseEvent } ; # [inline (never)] pub fn lipis_flag_icons_4_x_3_uy (width : String , height : String , onclick : Option < Callback < MouseEvent >>) -> Html { yew :: html ! { < svg xmlns = "http://www.w3.org/2000/svg" data - license = "From https://github.com/lipis/flag-icons - Licensed under MIT" { width } { height } { onclick } fill = "currentColor" id = "flag-icons-uy" viewBox = "0 0 640 480" > < path fill = "#fff" d = "M0 0h640v480H0z" /> { "
  ...
  " } < g fill = "#fcd116" stroke = "#000" stroke - miterlimit = "20" stroke - width = ".6" transform = "translate(133.3 133.3) scale(2.93333)" > { "
    " } < g id = "c" > { "
      " } < g id = "b" > { "
        ...
        " } </ g > { "
        " } < use href = "#a" width = "100%" height = "100%" transform = "rotate(45)" /> { "
      " } </ g > { "
      " } < use href = "#b" width = "100%" height = "100%" transform = "rotate(90)" /> { "
    " } </ g > { "
    " } < use href = "#c" width = "100%" height = "100%" transform = "scale(-1)" /> { "
    " } < circle r = "11" /> { "
  " } </ g > { "
