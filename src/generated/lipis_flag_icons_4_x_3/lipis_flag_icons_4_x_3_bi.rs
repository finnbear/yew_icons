use yew :: { Callback , Html , MouseEvent } ; # [inline (never)] pub fn lipis_flag_icons_4_x_3_bi (width : String , height : String , onclick : Option < Callback < MouseEvent >>) -> Html { yew :: html ! { < svg xmlns = "http://www.w3.org/2000/svg" data - license = "From https://github.com/lipis/flag-icons - Licensed under MIT" { width } { height } { onclick } fill = "currentColor" id = "flag-icons-bi" viewBox = "0 0 640 480" > < defs > { "
    " } < clipPath id = "bi-a" > { "
      " } < path fill - opacity = ".7" d = "M-90.5 0H592v512H-90.5z" /> { "
    " } </ clipPath > { "
  " } </ defs > { "
  " } < g fill - rule = "evenodd" transform = "translate(84.9) scale(.9375)" > { "
